...
// sparql.ts はフェーズ2d で core/sparql.ts へ移動予定。それまで lib 経由。
...
// DatasetProfile.geohashNearby.narrowScope 実装(design §3.11)。
// 入力座標の geohash から候補都道府県を1つ引き当て、SPARQL 近傍検索の
// スコープ(ic:都道府県 = <label>)として返す。引き当てられなければ null。
// フェーズ3で core/helpers/geohashNearby.ts + loa の reverse_geocode Tool が使う。
export async function loaNarrowScope(
  inputGeohash: string
  ...
  try {
    ...
    return { predicate: `${IC_PREFIX}都道府県`, value: match.label };
  } catch {
    return null;
  }
}

const IC_PREFIX = "http://imi.go.jp/ns/core/rdf#";
