...
export function simplifyGeometry(
  ...
  if (geometry.type === "LineString") {
    return { type: "LineString", coordinates: simplifyLine(geometry.coordinates, tolerance) };
  }
...
// An open line only needs its two endpoints to stay valid, so RDP applies
// directly with no closed-ring floor. douglasPeucker already returns inputs
// of length <= 2 unchanged.
